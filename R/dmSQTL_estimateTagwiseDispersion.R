         ...
         optimize={
           ...
           disp_list <- bplapply(gene_list, function(g){
             ...
             for(i in 1:nrow(snps)){
               ...
               if(is.null(dm_profileLikTagwise(gamma0 = gamma0, y = yg, ngroups=ngroups, lgroups=lgroups, igroups=igroups, disp_adjust = disp_adjust, prop_mode = prop_mode, prop_tol = prop_tol, verbose = verbose))){
                 disp[i] <- NA
                 next
...
#### set NA for genes that tagwise disp could not be calculated            
           ...
           dispersion <- split(dispersion, genesAll[, 1])
           dispersion <- dispersion[gene_list]
