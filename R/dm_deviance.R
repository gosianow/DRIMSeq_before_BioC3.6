...
dm_devG <- function(pi, gamma0, y){
  ...
  ll_mod <- sum( colSums( lgamma(y + pi * gamma0) - lgamma(pi * gamma0) , na.rm = TRUE ) )
  
  pi_sat <- y/colSums(y)
  
  ll_sat <- sum( colSums( lgamma(y + pi_sat * gamma0) - lgamma(pi_sat * gamma0) , na.rm = TRUE) ) # Inf for y = 0
