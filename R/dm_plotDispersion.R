...
dm_plotDispersion <- function(genewise_dispersion, mean_expression, nr_features = NULL, common_dispersion = NULL){
  ...
  if(!is.null(nr_features)){
    ...
    df <- data.frame(mean_expression = log10(mean_expression + 1), dispersion = log10(genewise_dispersion))
  ...
  if(!is.null(common_dispersion)){
    ggp <- ggp + geom_hline(yintercept = log10(common_dispersion), colour = "black", linetype = "dashed", size =  0.5)
  }
