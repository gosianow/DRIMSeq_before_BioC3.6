...
dm_plotProportions <- function(counts, group, pi_full = NULL, pi_null = NULL, 
  ...
  if(plot_type == "boxplot1"){
    ...
    ggp <- ggplot() +
      ...
      geom_jitter(data = prop_samp, 
        aes_string(x = "feature_id", y = "proportion", fill = "group", colour = "group"), 
        position = position_jitterdodge(dodge.width = 0.75), 
        alpha = 0.5, size = 2, show.legend = FALSE, na.rm = TRUE) +
      geom_boxplot(data = prop_samp, 
        aes_string(x = "feature_id", y = "proportion", colour = "group", fill = "group"), 
  ...
  if(plot_type == "boxplot2"){
    ...
    ggp <- ggplot() +
      ...
      theme(axis.text.x = element_text(angle = 0, vjust = 0.5), 
        ...
        axis.title=element_text(size=14, face="bold"), 
        plot.title = element_text(size=14), panel.grid.major = element_blank(), 
        legend.title = element_text(size = 14), 
        legend.text = element_text(size = 14)) +
  ...
  if(plot_type == "lineplot"){
    ...
    ggp <- ggplot() +
      ...
      geom_line(data = prop_samp, 
        aes_string(x = "feature_id", y = "proportion", group = "sample_id", colour = "group"), 
        size = 1.1) +
      scale_fill_manual(name = "Groups", values = values, breaks = names(values)) +
      scale_colour_manual(name = "Groups", values = values, breaks = names(values)) +
      xlab("Features") +
    ...
    if(!is.null(pi_null)){
      ggp <- ggp +
        geom_point(data = prop_est_null, 
          aes_string(x = "feature_id", y = "proportion", fill = "group"), 
          size = 3.5, shape = 22, show.legend = FALSE) +
        guides(colour=FALSE)
    }
    ...
    if(!is.null(pi_full)){
      ggp <- ggp + 
        geom_point(data = prop_est_full, 
          aes_string(x = "feature_id", y = "proportion", group = "group", fill = "group"), 
          size = 3, shape = 23) +
        guides(colour=FALSE)
    }
  ...
  if(plot_type == "ribbonplot" & !is.null(pi_full)){
    ...
    for (i in 1:(nlevels(group) - 1)){
      # i = 2
      prop_est_full_ribbon <- prop_est_full_order[prop_est_full_order$group %in% 
          levels(prop_est_full_order$group )[c(i, i+1)], ]
      prop_est_full_ribbon$group <- factor(prop_est_full_ribbon$group)
      prop_est_full_ribbon$cumsum <- matrix(t(aggregate(prop_est_full_ribbon[,"proportion"], 
        by = list(group = prop_est_full_ribbon$group), cumsum)[, -1]), ncol = 1)
      ...
      gr[[i]] <- geom_ribbon(data = prop_est_full_ribbon, 
        aes_string(x = "x", ymin = "ymin", ymax = "ymax", group = "feature_id", fill = "feature_id"), 
        alpha = 0.3) 
