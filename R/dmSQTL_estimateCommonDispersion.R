...
dmSQTL_estimateCommonDispersion <- function(counts, genotypes, samples, disp_adjust = TRUE, disp_interval = c(0, 1e+5), disp_tol = 1e-01, prop_mode = "constrOptimG", prop_tol = 1e-12, verbose=FALSE, BPPARAM = MulticoreParam(workers=1)){
	
	cat("Estimating common dispersion.. \n")
	
	### keep only one SNP per gene
	genotypes <- lapply(genotypes, function(g){ g[1, , drop = FALSE]})  
