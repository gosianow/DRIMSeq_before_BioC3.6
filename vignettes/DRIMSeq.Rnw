...
% The statistical details of \Rpackage{DRIMSeq} are presented in our paper \fixme{citation to DRIMSeq paper}.
...
In this vignette, we present how one could perform differential splicing analysis and sQTL analysis with \Rpackage{DRIMSeq} package. We use small data sets so that the whole pipelines can be run within few minutes in \R{} on a single core computer. In practice, the package is designed to take advantage of multicore computing for larger data sets.
...
In general, sQTL analyses are more heavy computationally than differential splicing analysis because you have to do the analysis for every SNP that is in the surrounding of a gene which can be hundreds. You may consider performing sQTL analysis for small chunks of genome, for example, per chromosome.
...
In order to do the analysis, we create a \Rclass{dmDSdata} object, which contains feature counts and information about grouping samples into conditions. With each step of the pipeline, additional elements are added to this object. At the end of the analysis, the object contains results from all the steps, such as dispersion estimates, proportions estimates, likelihood ratio statistics, p-values, adjusted p-values. As new elements are added, the object also changes its name \Rclass{dmDSdata} $->$ \Rclass{dmDSdispersion} $->$ \Rclass{dmDSfit} $->$ \Rclass{dmDStest}, but each container inherits slots and methods available for the previous one.



\subsubsection{Loading pasilla data into R}

The counts obtained from \software{kallisto} and metadata are saved as text files in the \Rcode{extdata} directory of \Biocexptpkg{PasillaTranscriptExpr} package.
...
max_nr_isoforms <- max(width(d@counts))
@


You can also make a data summary plot, which is a histogram of the number of features per gene. For example, there are genes that have \Sexpr{max_nr_isoforms} isoforms.
...
Genes may have many transcripts or exons that are lowly expressed or not expressed at all. You can remove them using the \Rfunction{dmFilter} function. Filtering of lowly expressed features can be done at two levels: minimal expression using \Robject{min\_samps\_feature\_expr} and \Robject{min\_feature\_expr} parameters or minimal proportion with \Robject{min\_samps\_feature\_prop} and \Robject{min\_feature\_prop}.

In \emph{pasilla} assay we use a filtering based only on the feature absolute expression and parameters are adjusted according to the number of replicates per condition. Since we have 3 knock-down and 4 control samples, we set \Robject{min\_samps\_feature\_expr} equal 3. In this way, we allow a situation where a feature (here, a transcript) is expressed in one condition but may not be expressed at all in another one, which is a case of differential splicing. The level of feature expression is controlled by \Robject{min\_feature\_expr}. Our default value is set up to 10, which means that only the features that have at least 10 counts in at least 3 samples are kept for the downstream analysis.

Filtering at the gene level ensures that the observed feature ratios have some minimal reliability. Although, Dirichlet-multinomial model works on feature counts, and not on feature ratios, which means that it gives more confidence to the ratios based on 100 versus 500 reads than 1 versus 5, minimal filtering based on gene expression removes the genes with mostly zero counts and reduces the number of tests in multiple test correction. For \emph{pasilla} data we want that genes have at least 10 counts in all the samples: \Rcode{min\_samps\_gene\_expr = 7} and \Rcode{min\_gene\_expr = 10}.
...
This step of our pipeline is the most time consuming. Thus consider using \Rcode{BPPARAM = BiocParallel::MulticoreParam()} with more than one worker.
...
If \Rcode{out\_dir = NULL} in any of the plotting function from \Rpackage{DRIMSeq} package, \Rclass{ggplot} object is returned and it can be further modified. Here, we increase the size of points.
...
To obtain the results of likelihood ratio tests, you have to call the function \Rfunction{results}, which returns a data frame with likelihood ratio statistics, degrees of freedom, p-values and Benjamini and Hochberg adjusted p-values for each gene.
...
We use the subsets of data defined in \Biocexptpkg{GeuvadisTranscriptExpr} package.
...
In the sQTL analysis, an initial data object \Robject{d} is of \Robject{dmSQTLdata} class and, additionally to feature counts, it contains genotypes of SNPs that are in some surrounding of genes. This surrounding is defined with the parameter \Rcode{window}. In order to find out which SNPs should be tested with which genes we can use the \Rfunction{dmSQTLdataFromRanges} functions which requires as an input the location of genes and SNPs stored as \Rclass{GRanges} objects. In the case you already know the match of SNPs and genes, you can use the \Rfunction{dmSQTLdata} function.
...
The filtering step eliminates genes and features with low expression, as in the differential splicing analysis (see section \ref{DS_filtering}). Additionally, it filters out the SNPs/blocks that do not define at least two genotypes where each of them is present in at least \Robject{minor\_allele\_freq} individuals. Usually, \Robject{minor\_allele\_freq} is equal to more or less 5\% of total sample size.
...
Again, this step of our pipeline is the most time consuming. Thus consider using \Rcode{BPPARAM = BiocParallel::MulticoreParam()} with more than one worker when performing real data analysis.
...
As in section \ref{DS_testing}, \Rfunction{dmTest} function fits null model proportions and performs the likelihood ratio test.
...
You can plot the observed and estimated Dirichlet-multinomial model feature ratios for the sQTLs of interest. When the sample size is large, we recommend using box plots as a \Rcode{plot\_type}. Here, we plot an sQTL with the lowest p-value.
