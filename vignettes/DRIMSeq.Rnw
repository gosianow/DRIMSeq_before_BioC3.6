...
library(knitr)
opts_chunk$set(cache = FALSE, tidy = FALSE, tidy.opts = list(blank = FALSE), highlight=FALSE, out.width = "7cm", out.height = "7cm", fig.align = "center")
@
...
max_nr_isoforms <- max(elementLengths(dm_counts(d)))
@
...
If \Rcode{out\_dir = NULL} in any of the plotting functions from \Rpackage{DRIMSeq} package, a \Rclass{ggplot} object is returned, and it can be further modified using \CRANpkg{ggplot2} package. Here, we increase the size of points.
